...
export default function App() {
  ...
  const handleAddProductSubmit = async () => {
    ...
    if (!name || !lotNum || isNaN(qty) || qty < 0 || !exp || isNaN(price)) {
    ...
    if (existingProd) {
      // Product already exists: Merge the lot!
      const updatedProduct: Producto = {
        ...existingProd,
        nombre: name || existingProd.nombre,
        descripcion: desc || existingProd.descripcion || "",
        lotes: [...existingProd.lotes, newLot],
        updatedAt: Date.now()
      };
      await saveProductDoc(updatedProduct);
      showToast(`Lote registrado en el producto existente "${name}".`);
    } else {
      ...
      showToast(`"${name}" añadido correctamente al inventario con código ${code}.`);
    }
                          ...
                          <div className="absolute left-0 right-0 mt-1 bg-white border border-slate-200 rounded-xl shadow-lg z-50 max-h-48 overflow-y-auto">
                            {filteredSuggestions.map((prod) => (
                              ...
                              >
                                ...
                                <span className="text-[10px] text-slate-400 font-mono">{prod.codigo}</span>
                              </button>
                            ))}
                          </div>
                        );
