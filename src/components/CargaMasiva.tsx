              ...
              <div className="overflow-x-auto max-h-[580px]">
                <table className="w-full text-xs text-left border-collapse">
                  <thead className="sticky top-0 z-10 bg-slate-100 text-slate-700 font-bold uppercase tracking-wider text-[11px] border-b border-slate-200 shadow-xs">
                    <tr>
                      <th className="sticky top-0 left-0 z-30 py-3.5 px-3 text-center w-[48px] min-w-[48px] max-w-[48px] bg-slate-100 border-r border-slate-200">#</th>
                      <th className="sticky top-0 left-[48px] z-30 py-3.5 px-3 min-w-[220px] bg-slate-100 border-r border-slate-300 shadow-[4px_0_8px_-2px_rgba(0,0,0,0.08)]">Nombre (Excel)</th>
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {filteredPreview.length === 0 ? (
                      ...
                      filteredPreview.map((item, idx) => {
                        ...
                        const stickyCellBg = isManual
                          ? "bg-[#f5f6ff] group-hover:bg-[#ebedff]"
                          : isExisting
                          ? "bg-[#f0fdf6] group-hover:bg-[#dcfce9]"
                          : isSameBatch
                          ? "bg-[#faf5ff] group-hover:bg-[#f3e8ff]"
                          : "bg-[#f0f9ff] group-hover:bg-[#e0f2fe]";
                        ...
                        return (
                          <tr
                            ...
                            className={`group transition ${
                              isManual
                                ? "bg-indigo-50/20 hover:bg-indigo-50/70"
                                : isExisting
                                ? "bg-emerald-50/15 hover:bg-emerald-50/70"
                                : isSameBatch
                                ? "bg-purple-50/15 hover:bg-purple-50/70"
                                : "bg-sky-50/15 hover:bg-sky-50/70"
                            }`}
                          >
                            {/* 1. # Fila (Inmovilizado / Sticky) */}
                            <td className={`sticky left-0 z-20 py-3 px-3 text-center font-mono font-bold text-slate-400 text-[11px] w-[48px] min-w-[48px] max-w-[48px] border-r border-slate-200/60 transition-colors ${stickyCellBg}`}>
                              {item.row}
                            </td>

                            {/* 2. Nombre (Excel) (Inmovilizado / Sticky) */}
                            <td className={`sticky left-[48px] z-20 py-3 px-3 min-w-[220px] border-r border-slate-300 shadow-[4px_0_8px_-2px_rgba(0,0,0,0.08)] transition-colors ${stickyCellBg}`}>
                              <div className="space-y-0.5">
                                <span className="font-extrabold text-slate-900 block text-xs">
                                  {item.nombre}
                                </span>
                                {item.codigoOriginal && (
                                  <span className="inline-block text-[10px] font-mono text-slate-500 bg-white/90 border border-slate-200 px-1.5 py-0.2 rounded shadow-2xs">
