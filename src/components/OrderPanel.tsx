...
import { ShoppingCart, Send, Trash2, Search, Plus, X, FileText, FileDown } from "lucide-react";
import { exportOrderToPDF, exportOrderToWord } from "../utils/exportUtils";
...
export default function OrderPanel({
  ...
  const activeCatalogName = availableCatalogos.find((c) => c.id === orderCatalogId)?.nombre || "Medicamentos e Insumos";
        ...
        <div className="flex flex-wrap items-center gap-2">
          {orderItems.length > 0 && (
            <>
              ...
              <button
                onClick={() => exportOrderToPDF(orderItems, activeCatalogName)}
                className="rounded-xl border border-rose-500 bg-rose-50 text-rose-700 hover:bg-rose-100 px-3 py-2 text-xs font-bold transition flex items-center gap-1.5 cursor-pointer"
              ...
              <button
                onClick={() => exportOrderToWord(orderItems, activeCatalogName)}
                className="rounded-xl border border-blue-500 bg-blue-50 text-blue-700 hover:bg-blue-100 px-3 py-2 text-xs font-bold transition flex items-center gap-1.5 cursor-pointer"
                title="Exportar Pedido a Word"
              >
                  ...
                  <tr key={idx} className="hover:bg-slate-50/50 bg-white transition">
                    ...
                    <td className="px-4 py-3 text-right">
                      <input
                        type="number"
                        min="1"
                        value={item.cantidadAprobada === 0 ? "" : item.cantidadAprobada}
                        onFocus={(e) => e.target.select()}
                        onChange={(e) => onUpdateOrderItemQuantity(idx, e.target.value === "" ? 0 : Number(e.target.value))}
                        placeholder="0"
